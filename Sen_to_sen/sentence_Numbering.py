...
def __aggregate(text,attribute_list):

    corpus = []
    for i,attributes in enumerate(attribute_list):
        tmp_text = text[i]
        for attribute in attributes:
            if 'original' in attribute.keys():
                tmp_text= __aggregate_by_original(tmp_text,attribute)
            elif attribute['end'] is not None:
                tmp_text=__aggregate_by_index(tmp_text,attribute)
...
def extract_attribute(tmp_dic):
    ...
    return __aggregate(text,attribute_list),attribute_list,text
...
def write_training_data(input_data, output_data):
    file = open('input_data.pkl','wb')
    pickle.dump(input_data, file)
    file.close()
    file = open('output_data.pkl','wb')
...
write_index(word_set,word_index,index_word)
...
write_training_data(input_data,output_data)
